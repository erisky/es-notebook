...
struct lws  *wsi_fsc=NULL;
static struct lws *wsi_multi[512];
static volatile int force_exit;
static unsigned int opts, rl_multi[512];
			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_CLIENT_WRITEABLE:
        ...
        sprintf(&buf[LWS_PRE], "HAHA:[%d]\n", count++);
        n = lws_write(wsi, &buf[LWS_PRE], strlen(&buf[LWS_PRE]), LWS_WRITE_TEXT);
        printf("sent:%d \n", n);
		break;
...
int main(int argc, char **argv)
{
	...
	while (!force_exit) {
        if (initial == 0 ) {
            ...
            wsi_fsc = NULL;
        	lwsl_notice("FSC: connecting\n");
		    i.protocol = protocols[PROTOCOL_FILE_CLIENT].name;
			i.pwsi = &wsi_fsc;
            i.userdata = malloc(sizeof(struct per_session_file_client_info));
            memset(i.userdata ,0, sizeof(struct per_session_file_client_info));
			lws_client_connect_via_info(&i);
        
            initial = 1;
            printf ("wsi_fc:%x", wsi_fsc);
        }        
#if 0        
        else if (wsi_fsc) {
            //lws_context_user 
            psc =  (struct per_session_file_client_info *) lws_wsi_user(wsi_fsc);
            //lws_get_context(wsi_fsc)
            //lws_get_context
            //printf("%p", wsi_fsc->user_space);
            //printf("%p", wsi_fsc->rxflow_buffer);
            
            if ((psc->state == FSC_DONE) || (psc->state == FSC_ERROR)) {
                force_exit = 1;
            }
        }
        else {
        ...
        if (wsi_dumb && ratelimit_connects(&rl_dumb, 1)) {
